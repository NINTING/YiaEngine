...
namespace YiaEngine
{
	namespace Graphic
	{
		...
		void RenderBuffer::CreateView(DXGI_FORMAT format, UINT arraySize, UINT numMips)
		{
			...
			if (srvHandle.GetCpuAddress()->ptr == ADDRESS_UNKOWN)
			{
				srvHandle = g_CpuDescriptorAllocator[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV].Alloc(1);
				rtvHandle = g_CpuDescriptorAllocator[D3D12_DESCRIPTOR_HEAP_TYPE_RTV].Alloc(1);
			}
